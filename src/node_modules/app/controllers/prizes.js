...
module.exports = (service) => ({
  ...
  getDiffs: async (p, { since }) => {
    const newTickets = await service.db.collection('tickets')
      .where('created', '>', new Date(since))
      .get();

    return uniqBy(await Promise.all(newTickets.docs.map(async (d) => {
      const prize = await d.data().prize.get();
      ...
      return Object.assign({}, prize.data(), {
        id: prize.id,
        committedTickets: tickets.docs.length,
      });
    })), 'id');
  },
});